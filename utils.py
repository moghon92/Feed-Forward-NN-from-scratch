...
def load_mnist_trainval():
	...
	#############################################################################
	train_slice = int(0.8 * len(data))
	
	train_data = data[:train_slice]
	train_label = label[:train_slice]
	
	val_data = data[train_slice:]
	val_label = label[train_slice:]
	
	#assert round(len(train_data) / len(data), 1) == 0.8, "tain valid split error"
	#assert round(len(train_label) / len(label), 1) == 0.8, "tain valid split error"
	#assert round(len(val_data) / len(data), 1) == 0.2, "tain valid split error"
	#assert round(len(val_label) / len(label), 1) == 0.2, "tain valid split error"
	#############################################################################
	#                              END OF YOUR CODE                             #
	#############################################################################
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
	...
	#    batch size                                                             #
	#############################################################################
	# take copies of originals
	shuffled_data = data.copy()
	shuffled_label = label.copy()
	
	# schuffle them in the same order
	if shuffle == True:
		combined_shuffle = list(zip(shuffled_data, shuffled_label))
		random.shuffle(combined_shuffle)
		shuffled_data, shuffled_label = zip(*combined_shuffle)
	
	# calculate num minibatches needed 
	n_minibatches = len(shuffled_data) // batch_size
	...
	for i in range(n_minibatches):
		...
		y_batch = shuffled_label[i * batch_size : (i + 1) * batch_size]
		batched_data.append(np.array(X_batch))
		batched_label.append(np.array(y_batch))
		
	# if num data points is indivisble by num batches then collect remaining points in a smaller batch
	if len(shuffled_data) % batch_size != 0:
		X_batch = shuffled_data[(i + 1)  * batch_size : ]
		y_batch = shuffled_label[(i + 1) * batch_size : ]
		batched_data.append(np.array(X_batch))
		batched_label.append(np.array(y_batch))
